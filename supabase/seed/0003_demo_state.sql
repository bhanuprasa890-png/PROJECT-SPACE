...
--
-- DEMO / SIMULATED DATA: 10 service notices, 3 demo users (commuter, operator,
-- admin) and ~1k journey searches for a simulated Indian city network.
-- =============================================================================
...
values
  ('ALT-1001', 'AG-MERIDIAN', 'LN-M1', 'STN-02', 'critical', 'disruption',
   'Signal failure at Egmore Junction',
   'Metro Line 1 trains are running every 12 minutes between Chennai Central and Madhavaram while engineers replace a trackside signal. Expect platform crowding at Egmore Junction until service normalises.',
  ...
  ('ALT-1002', 'AG-MERIDIAN', 'LN-M2', 'STN-01', 'major', 'crowding',
   'Crush load forecast: Chennai Central northbound',
   'TransitPulse predicts occupancy above 105% on northbound Metro Line 2 services between 17:20 and 18:40. Consider boarding two services later or switching to the BR1 BRT at Tidel Park.',
  ...
  ('ALT-1003', 'AG-MERIDIAN', 'LN-B12', null, 'minor', 'delay',
   'MTC 21G running 4–7 minutes late',
   'Roadworks on the Tambaram corridor are delaying southbound services. Next departures from Anna University are adjusted in the journey planner.',
  ...
  ('ALT-1004', 'AG-MERIDIAN', 'LN-F2', null, 'major', 'weather',
   ...
   'Ennore–Port ferry services are suspended until sea conditions improve. Replacement buses run from Ennore Ferry Terminal to Chennai Port every 15 minutes.',
  ...
  ('ALT-1005', 'AG-MERIDIAN', 'LN-T4', null, 'info', 'service_change',
   'Heritage Tram weekend timetable from Saturday',
   'Trams will run every 14 minutes on Saturdays and Sundays while track renewal works continue near Chepauk. The first tram leaves Ennore Ferry Terminal at 06:30.',
   now() + interval '1 day', now() + interval '9 days', 'scheduled', 'Service Planning', 0),

  ('ALT-1006', 'AG-MERIDIAN', 'LN-BR1', 'STN-13', 'minor', 'crowding',
   'Madhavaram boarding pressure this evening',
   'Expected occupancy above 85% on the 18:05 and 18:11 BRT departures from Madhavaram because of the trade fair at the exhibition grounds. An additional vehicle has been allocated.',
   now() - interval '25 minutes', now() + interval '3 hours', 'active', 'TransitPulse AI', 5640),

  ('ALT-1007', 'AG-MERIDIAN', 'LN-M2', null, 'info', 'maintenance',
   'Track inspection between Chepauk and T. Nagar tonight',
   'Planned Metro Line 2 inspection from 01:00 to 04:00. The last scheduled service is unaffected; overnight replacement buses will operate.',
   now() + interval '6 hours', now() + interval '11 hours', 'scheduled', 'Engineering', 0),

  ('ALT-1008', 'AG-MERIDIAN', 'LN-M1', 'STN-08', 'minor', 'delay',
   'T. Nagar escalator works — resolved',
   'Escalator 3 at T. Nagar is back in service. Residual delays on Metro Line 1 have cleared.',
   now() - interval '6 hours', now() - interval '2 hours', 'resolved', 'Station Team', 9980),

  ('ALT-1009', 'AG-MERIDIAN', 'LN-F2', 'STN-16', 'minor', 'delay',
   'Ferry F2 running to a 25-minute headway',
   'A single vessel is in service between Ennore Ferry Terminal and Chennai Port while the second boat is inspected. Expect a 25-minute wait at both piers.',
   now() - interval '55 minutes', now() + interval '4 hours', 'active', 'Operations Desk', 2140),

  ('ALT-1010', 'AG-MERIDIAN', 'LN-B12', 'STN-12', 'info', 'service_change',
   'Extra 21G services from Tambaram on weekday mornings',
   'From Monday, two additional 21G departures leave Tambaram at 08:05 and 08:20 to ease morning boarding pressure towards Anna University and Central.',
   now() + interval '2 days', now() + interval '30 days', 'scheduled', 'Service Planning', 0);

-- -----------------------------------------------------------------------------
-- Demo rider
-- -----------------------------------------------------------------------------
insert into rider_profiles (
  id, agency_id, display_name, email, role, home_stop_id, work_stop_id,
  ...
  ('profile-ava', 'AG-MERIDIAN', 'Ananya Raman', 'ananya.raman@example.com', 'commuter', 'STN-12', 'STN-04',
   0.780, 12, 1, '{metro,tram,brt,bus}',
   true, true, false, 0.820,
   '22:30', '06:30', 'dark', 'metric', 'en', true),
  ('profile-ops', 'AG-MERIDIAN', 'Chennai Control Room', 'control@chennaitransit.example', 'operator', 'STN-01', 'STN-01',
   1.050, 8, 2, '{metro,bus,tram,brt,ferry}',
   true, false, false, 0.900,
   null, null, 'midnight', 'metric', 'en', true),
  ('profile-admin', 'AG-MERIDIAN', 'Karthik Iyer', 'karthik.iyer@example.com', 'admin', 'STN-01', 'STN-01',
   0.900, 10, 2, '{metro,bus,tram,brt,ferry}',
   false, true, false, 0.900,
   null, null, 'dark', 'metric', 'en', false);
...
insert into watchlist (id, profile_id, label, origin_stop_id, destination_stop_id, depart_time, days, avoid_crowded, notify) values
  ...
  ('WL-2003', 'profile-ava', 'Saturday shopping', 'STN-12', 'STN-08', '10:30', '{sat}',                 true,  false),
